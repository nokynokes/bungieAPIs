...
var $author$project$Main$init = F3(
	function (maybeFlags, url, navKey) {
		...
		var _v0 = $truqu$elm_oauth2$OAuth$AuthorizationCode$PKCE$parseCode(url);
		switch (_v0.$) {
			case 'Error':
				var error = _v0.a;
				return _Utils_Tuple2(
			...
			default:
				...
				if (maybeFlags.$ === 'Nothing') {
					...
					return (!_Utils_eq(
						...
						{
							flow: A2($author$project$Main$Authorized, code, flags.codeVerifier),
							redirectUri: redirectUri
						},
						$elm$core$Platform$Cmd$batch(
							_List_fromArray(
								[
									A2($andrewMacmurray$elm_delay$Delay$after, 750, $author$project$Main$AccessTokenRequested),
									clearUrl
								])));
				}
		}
...
var $author$project$Main$gotAccessToken = F2(
	function (model, authResponse) {
		if (authResponse.$ === 'Err') {
			if (authResponse.a.$ === 'BadBody') {
				var body = authResponse.a.a;
				...
				if (_v1.$ === 'Ok') {
					...
					return _Utils_Tuple2(
						_Utils_update(
							...
							{
								flow: $author$project$Main$Errored(
									$author$project$Main$ErrAuthentication(err))
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					return _Utils_Tuple2(
			...
			var token = authResponse.a.token;
			return _Utils_Tuple2(
				_Utils_update(
					model,
					{
...
var $author$project$OAuthInfo$defaultHttpsUrl = {fragment: $elm$core$Maybe$Nothing, host: '', path: '', port_: $elm$core$Maybe$Nothing, protocol: $elm$url$Url$Https, query: $elm$core$Maybe$Nothing};
var $author$project$OAuthInfo$oauthConfig = {
	authorizationEndpoint: _Utils_update(
		$author$project$OAuthInfo$defaultHttpsUrl,
		{host: 'bungie.net', path: '/en/OAuth/Authorize'}),
	clientId: '38809',
	scope: _List_Nil,
	...
	function (model, bytes) {
		...
		if (_v0.$ === 'Nothing') {
			...
			var authorization = A3(
				...
				_List_fromArray(
					[
						A2($elm$url$Url$Builder$string, 'client_id', $author$project$OAuthInfo$oauthConfig.clientId),
						A2($elm$url$Url$Builder$string, 'response_type', 'code'),
						A2($elm$url$Url$Builder$string, 'state', state)
					]));
			return _Utils_Tuple2(
				model,
				$elm$browser$Browser$Navigation$load(authorization));
		}
	});
				...
				case 'Authorized':
					switch (_v0.b.$) {
						case 'AccessTokenRequested':
							...
							return _Utils_Tuple2(
								model,
								A4($author$project$Main$getAccessToken, $author$project$OAuthInfo$oauthConfig, model.redirectUri, code, codeVerifier));
						case 'GotAccessToken':
							var _v6 = _v0.a;
							var authResponse = _v0.b.a;
							return A2($author$project$Main$gotAccessToken, model, authResponse);
						default:
							break _v0$4;
					}
				default:
...
var $author$project$Main$view = function (model) {
				...
				return _List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text(
								'Success! token is: ' + $truqu$elm_oauth2$OAuth$tokenToString(token))
							]))
					]);
			default:
				var error = _v0.a;
	...
	return {body: html, title: 'Bungie NET Auth'};
};
